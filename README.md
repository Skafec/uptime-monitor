...
- **T-2 — No edit-monitor UI [FIXED].** Added a collapsible "Edit monitor" form on the detail page (`components/EditMonitorForm.tsx`) to change name, URL, and check interval, wired to `PATCH /api/monitors/[id]`. Interval is Pro-gated, and `interval_minutes` is only sent when changed so a free user editing name/URL doesn't hit the Pro-only 403. This also fixes the "can't move an existing monitor to 1-minute after upgrading" gap found in testing.

...
- **T-1 — `isPublicUrl` false-positive on Vercel [high].** The SSRF guard (from P0-2) rejected `https://example.com` in the deployed runtime (recorded as `down`, `0 ms`, `null` status_code — i.e. rejected before fetch), while `https://google.com` passed. It can therefore wrongly reject **legitimate public customer URLs**. Suspected causes: DNS-lookup failure is treated as "not public," and/or IPv6 handling flags valid public addresses. **Fix before onboarding real users.** Likely fix: don't fail-closed on a transient DNS error, and tighten `isPrivateIpv6` to real reserved ranges only. Files: `lib/monitor.ts`.
